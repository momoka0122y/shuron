...
def plot_box_and_cdf(data_dualstack, data_clat, data_v6only, title_prefix, save_path_prefix):
    # Function to create box plot
    def create_box_plot(log_scale=False):
        plt.figure(figsize=(10, 6))
        boxplot_elements = plt.boxplot([data_dualstack['MedianTime'], data_clat['MedianTime'], data_v6only['MedianTime']],
                                       labels=['Dualstack', 'CLAT', 'v6only-capable'],
    ...
    def create_cdf_plot(log_scale=False):
        plt.figure(figsize=(10, 6))
        for data, label in zip([data_dualstack, data_clat, data_v6only], ['Dualstack', 'CLAT', 'v6only-capable']):
...
plot_box_and_cdf(data_dualstack, data_clat, data_v6only, "Twitter Analysis", "twitter1/twitter_analysis")
